...
@interface dbfTests : XCTestCase

@end

@implementation dbfTests
...
- (void)testOpenClose
{
    DBF_DATA	*pObj = OBJ_NIL;
   
    pObj = dbf_Alloc(0);
    XCTAssertFalse( (OBJ_NIL == pObj), @"Could not alloc  DBF_DATA" );
    pObj = dbf_Init( pObj );
    XCTAssertFalse( (OBJ_NIL == pObj), @"Could not init  DBF_DATA" );
    if (pObj) {
        obj_Release(pObj);
        pObj = OBJ_NIL;
    }
