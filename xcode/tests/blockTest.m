...
- (void)testOpenClose
{
    BLOCK_DATA	*pObj = OBJ_NIL;
    uint32_t    size = 0;
   
    pObj = block_NewWithSizes(0, 32);
    XCTAssertFalse( (OBJ_NIL == pObj) );
    if (pObj) {
        size = block_getHeaderSize(pObj);
        XCTAssertTrue( (0 == size) );
        size = block_getDataOffset(pObj);
        XCTAssertTrue( (0 == size) );
        size = block_getDataSize(pObj);
