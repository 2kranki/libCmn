...
)
{
    ...
    if (pObj) {
        ...
        for (i = 0; i < ntests; ++i) {
            ...
            m = TRegex_Match(pObj, pattern, text, &length);
            if (fDump) {
                printResult(pObj, i, pattern, text, mCheck, mLength, m, length);
            }
...
TINYTEST_START_SUITE(test_TRegex);
    TINYTEST_ADD_TEST(test_TRegex_Test02,setUp,tearDown);
