...
)
{
    ...
    pObj = I16Matrix_Alloc( );
    TINYTEST_FALSE( (OBJ_NIL == pObj) );
    pObj = I16Matrix_Init( pObj );
    TINYTEST_FALSE( (OBJ_NIL == pObj) );
    if (pObj) {

        //obj_TraceSet(pObj, true);       
        fRc = obj_IsKindOf(pObj, OBJ_IDENT_I16MATRIX);
        TINYTEST_TRUE( (fRc) );
...
TINYTEST_MAIN_SINGLE_SUITE(test_I16Matrix);
