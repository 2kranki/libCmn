...
ERESULT         Test_BPTLeaf_OpenClose (
    TEST_DATA       *this,
    const
    char            *pTestName
)
{
    ...
    pObj = BPTLeaf_Init( pObj );
    TestForNotNull(pObj, "Missing Test Init() object");
    if (pObj) {

        //obj_TraceSet(pObj, true);       
        fRc = obj_IsKindOf(pObj, OBJ_IDENT_BPTLEAF);
...
{
    ERESULT         eRc = ERESULT_SUCCESS;
    BPTLEAF_DATA       *pObj1 = OBJ_NIL;
    BPTLEAF_DATA       *pObj2 = OBJ_NIL;
    bool            fRc;
#if defined(BPTLEAF_JSON_SUPPORT) && defined(XYZZY)
    ...
    if (pObj1) {
        ...
        fRc = obj_IsKindOf(pObj1, OBJ_IDENT_BPTLEAF);
        TestForTrue(fRc, "Failed Ident Test");
        
        // Test assign.
        pObj2 = BPTLeaf_New();
        TestForNotNull(pObj2, "Missing copied object");
        eRc = BPTLeaf_Assign(pObj1, pObj2);
        TestForFalse((ERESULT_FAILED(eRc)), "Assignment failed");

        fRc = obj_IsKindOf(pObj2, OBJ_IDENT_BPTLEAF);
        TestForTrue(fRc, "Failed Ident Test");
        //iRc = BPTLeaf_Compare(pObj1, pObj2);
        ...
        // Test json support.
#if defined(BPTLEAF_JSON_SUPPORT) && defined(XYZZY)
        pStr = BPTLeaf_ToJson(pObj1);
        TestForNotNull(pStr, "Missing JSON output");
        fprintf(stderr, "JSON: %s\n", AStr_getData(pStr));
        pObj2 = BPTLeaf_NewFromJsonString(pStr);
...
)
{
    ...
    pObj = BPTLeaf_NewWithSizes(blockSize, 22, 4, 0, &FakeObj);
    TestForNotNull(pObj, "Missing Test object");
    if (pObj) {

        pObj->pReq = (void *)BlockRequest; // Simulate BPT::BlockRequest.
        //obj_TraceSet(pObj, true);       
        pRsvd = (BPTLEAF_RSVD *)BlkdRcds16_getReserved((BLKDRCDS16_DATA *)pObj);
        fRc = obj_IsKindOf(pObj, OBJ_IDENT_BPTLEAF);
        TestForTrue(fRc, "Failed Ident Test");
        TestForTrue((pObj->lbn == 22), "Failed Test");
        TestForTrue((BPTLeaf_getKeyLen(pObj) == 4), "Failed Test");
        TestForTrue((BPTLeaf_getKeyOff(pObj) == 0), "Failed Test");
        Test_Dump(blockSize, BlkdRcds16_getData((BLKDRCDS16_DATA *)pObj), "Before Adds");
        fRc = BPTLeaf_Verify(pObj);
        TestForTrue(fRc, "Failed Test");

        eRc = BPTLeaf_Add(pObj, 4, "001");
        TestForSuccess("Failed Add - 001");
        fRc = BPTLeaf_Verify(pObj);
        TestForTrue(fRc, "Failed Test");

        eRc = BPTLeaf_Add(pObj, 4, "005");
        TestForSuccess("Failed Add - 005");
        Test_Dump(blockSize, BlkdRcds16_getData((BLKDRCDS16_DATA *)pObj), "After add 005");
        fRc = BPTLeaf_Verify(pObj);
        TestForTrue(fRc, "Failed Test");

        eRc = BPTLeaf_Add(pObj, 4, "003");
        TestForSuccess("Failed Add - 003");
        Test_Dump(blockSize, BlkdRcds16_getData((BLKDRCDS16_DATA *)pObj), "After add 003");
        fRc = BPTLeaf_Verify(pObj);
        TestForTrue(fRc, "Failed Test");

        eRc = BPTLeaf_Add(pObj, 4, "007");
        ...
        iRc = BPTLeaf_FindByKey(pObj, "001", &cData);
        TestForTrue((1 == iRc), "Failed Test");
        TestForTrue((0 == cData), "Failed Test");

        iRc = BPTLeaf_FindByKey(pObj, "002", &cData);
        TestForTrue((0 == iRc), "Failed Test");
        TestForTrue((1 == cData), "Failed Test");

        iRc = BPTLeaf_FindByKey(pObj, "003", &cData);
        TestForTrue((2 == iRc), "Failed Test");
        TestForTrue((0 == cData), "Failed Test");

        iRc = BPTLeaf_FindByKey(pObj, "004", &cData);
        TestForTrue((0 == iRc), "Failed Test");
        TestForTrue((2 == cData), "Failed Test");

        iRc = BPTLeaf_FindByKey(pObj, "005", &cData);
        TestForTrue((3 == iRc), "Failed Test");
        TestForTrue((0 == cData), "Failed Test");

        iRc = BPTLeaf_FindByKey(pObj, "006", &cData);
        TestForTrue((0 == iRc), "Failed Test");
        TestForTrue((3 == cData), "Failed Test");

        iRc = BPTLeaf_FindByKey(pObj, "007", &cData);
        TestForTrue((4 == iRc), "Failed Test");
        TestForTrue((0 == cData), "Failed Test");

        iRc = BPTLeaf_FindByKey(pObj, "000", &cData);
        TestForTrue((0 == iRc), "Failed Test");
        TestForTrue((0 == cData), "Failed Test");

        iRc = BPTLeaf_FindByKey(pObj, "009", &cData);
        TestForTrue((0 == iRc), "Failed Test");
        TestForTrue((4 == cData), "Failed Test");


        cData = 0;
        pData = BPTLeaf_GetNum(pObj, 1, &cData);
        ...
        cData = 0;
        pData = BPTLeaf_GetNum(pObj, 2, &cData);
        TestForNotNull(pData, "Failed Test");
        ...
        cData = 0;
        pData = BPTLeaf_GetNum(pObj, 3, &cData);
        TestForNotNull(pData, "Failed Test");
        ...
        cData = 0;
        pData = BPTLeaf_GetNum(pObj, 4, &cData);
        TestForNotNull(pData, "Failed Test");
        ...
        cData = 0;
        pData = data;
        eRc = BPTLeaf_Get(pObj, "003", sizeof(data), pData, &cData);
        ...
        cData = 0;
        pData = data;
        eRc = BPTLeaf_Get(pObj, "001", sizeof(data), NULL, &cData);
        ...
        cData = 0;
        pData = data;
        eRc = BPTLeaf_Get(pObj, "001", sizeof(data), pData, &cData);
        TestForSuccess("Failed Get - 001");
        TestForTrue((0 == strcmp("001", (char *)pData)), "Failed Test");
        TestForTrue((4 == cData), "Failed Test");
        ...
        cData = 0;
        pData = data;
        eRc = BPTLeaf_Get(pObj, "007", sizeof(data), pData, &cData);
        TestForSuccess("Failed Get - 007");
        TestForTrue((0 == strcmp("007", (char *)pData)), "Failed Test");
        TestForTrue((4 == cData), "Failed Test");


        eRc = BPTLeaf_Delete(pObj, "007");
        TestForSuccess("Failed Delete - 007");
        Test_Dump(blockSize, BlkdRcds16_getData((BLKDRCDS16_DATA *)pObj), "After delete 007");
        fRc = BPTLeaf_Verify(pObj);
        TestForTrue(fRc, "Failed Test");
        cData = BPTLeaf_getNumRecords(pObj);
        TestForTrue((3 == cData), "Failed Test");

        eRc = BPTLeaf_Delete(pObj, "003");
        TestForSuccess("Failed Delete - 003");
        Test_Dump(blockSize, BlkdRcds16_getData((BLKDRCDS16_DATA *)pObj), "After delete 003");
        fRc = BPTLeaf_Verify(pObj);
        TestForTrue(fRc, "Failed Test");
        cData = BPTLeaf_getNumRecords(pObj);
        TestForTrue((2 == cData), "Failed Test");
        ...
        {
            ASTR_DATA       *pStr = BPTLeaf_ToDebugString(pObj, 4);
            if (pStr) {
...
)
{
    ...
    pObj = BPTLeaf_NewWithSizes(blockSize, 22, 4, 0, &FakeObj);
    TestForNotNull(pObj, "Missing Test object");
    pObj2 = BPTLeaf_NewWithSizes(blockSize, 22, 4, 0, &FakeObj);
    TestForNotNull(pObj, "Missing Test object");
    if (pObj) {
...
)
{
    ...
    // Execute tests.
    TestExec("OpenClose", Test_BPTLeaf_OpenClose, NULL, NULL);
    //TestExec("Copy01", Test_BPTLeaf_Copy01, pTest, NULL, NULL);
    TestExec("Test01", Test_BPTLeaf_Test01, NULL, NULL);
    TestExec("Test02", Test_BPTLeaf_Test02, NULL, NULL);
