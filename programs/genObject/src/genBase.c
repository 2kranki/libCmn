...
extern "C" {
    ...
    ****************************************************************/
    ...
    GENBASE_DATA *      genBase_NewWithDict (
        NODEBTP_DATA        *pDict
    )
    {
        GENBASE_DATA        *this;
    ...
    )
    {
        ...
        while (fMore) {
            ...
            if (ERESULT_FAILED(eRc)) {
                ...
                do_replace:
                    pNode = nodeBTP_FindA(this->pDict, 0, AStr_getData(pName));
                    if (OBJ_NIL == pNode) {
                        obj_Release(pName);
...
)
{
    ...
    )
    {
        ...
        if ((OBJ_NIL == pLibInstalledPath) && this->pDict) {
            pNode = nodeBTP_FindA(this->pDict, 0, "LibInstalledPath");
