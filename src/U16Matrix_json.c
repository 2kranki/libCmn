...
extern "C" {
    ...
    )
    {
        ...
        if (pObject->m && pObject->n) {
            eRc = U16Matrix_Setup(pObject, pObject->m, pObject->n);
        }
        if (pObject->cElems && pObject) {
            ...
            for(i=0; i<pObject->cElems; i++) {
                ...
                if (pValue)
                    *pValue = (uint16_t)AStr_ToInt64(pStr);
            }
        }
    ...
    {
        ERESULT         eRc;
        U16MATRIX_DATA   *pObject = OBJ_NIL;
        const
        OBJ_INFO        *pInfo;
        //int64_t         intIn;
        //ASTR_DATA       *pWrk;

        pInfo = obj_getInfo(U16Matrix_Class());
        ...
        pObject = U16Matrix_New( );
        if (OBJ_NIL == pObject) {
            goto exit00;
        }
        
        eRc =  U16Matrix_ParseJsonFields(pParser, pObject);
    ...
    )
    {
        ...
        pObject = U16Matrix_ParseJsonObject(pParser);
    ...
    )
    {
        ...
        if (pStringA) {
            ...
            pObject = U16Matrix_NewFromJsonString(pStr);
            obj_Release(pStr);
            pStr = OBJ_NIL;
        }
    ...
    )
    {
        ...
        if (pStr) {
            ...
            eRc = U16Matrix_ToJsonFields(this, pStr);      
