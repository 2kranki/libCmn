...
extern "C" {
    ...
    {
        ERESULT         eRc;
        NODETREE_DATA   *pObject = OBJ_NIL;
        const
        OBJ_INFO        *pInfo;
        //int64_t         intIn;
        //ASTR_DATA       *pWrk;

        pInfo = obj_getInfo(NodeTree_Class());
    ...
    )
    {
        ...
        pObject = NodeTree_ParseJsonObject(pParser);
    ...
    )
    {
        ...
        if (pString) {
            ...
            pObject = NodeTree_NewFromJsonString(pStr);
            obj_Release(pStr);
            pStr = OBJ_NIL;
        }
